...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    //根据key取hash值的函数也要关注一下，它称之为“扰动函数”
    ...
    public V put(K key, V value) {
    	//先根据key，取得hash值。 再调用putVal方法插入节点
...
//  再看一下 往哈希表里插入一个节点的putVal函数,如果参数onlyIfAbsent是true，那么不会覆盖相同key的值value。如果evict是false，那么表示是在初始化时调用的
     ...
     * 否则，因为我们扩容成以前的两倍。
     * 在扩容时，要注意区分以前在哈希桶相同index的节点，现在是在以前的index里，还是
     * index+oldlength 里。
     */
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            if (node != null && (!matchValue || (v = node.value) == value ||
                ...
                //否则待删除节点在表中间
                	...
                	//p和e一起在变化，始终是相邻的，只是把待删除节点node的前置节点p.next指向node.next
                    p.next = node.next;
                ++modCount;//修改modCount
                --size;//修改size
    ...
    public V putIfAbsent(K key, V value) {
    	//只会往表中插入 key-value, 若key对应的value之前存在，不会覆盖。
